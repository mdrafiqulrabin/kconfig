#!/bin/bash
#SBATCH -J sb_v5_gcc482_pk1
#SBATCH -o sb_v5_gcc482_pk1.o%j.txt
#SBATCH -t 13:01:01
#SBATCH -p alipour
...
function compile_tc {
    ...
    # -O0
    out1=$tc-$vn.out
    /project/alipour/rabin/bin/gcc482-bin/bin/gcc -I /project/alipour/rabin/bin/csmith230-bin/include/csmith-2.3.0 -w -msse4.2 $tc -o $out1 2> /dev/null
    if [ $? -ne 0 ]; then
        m_cb1=$((m_cb1+1));
    else
    ...
    # -O3
    out2=$tc-$vn-$ol.out
    /project/alipour/rabin/bin/gcc482-bin/bin/gcc -O$ol -I /project/alipour/rabin/bin/csmith230-bin/include/csmith-2.3.0 -w -msse4.2 $tc -o $out2 2> /dev/null
    if [ $? -ne 0 ]; then
        m_cb2=$((m_cb2+1));
    else
...
function exec_csmith {
    c=$((((c+1))%n))
    args=`python /project/alipour/rabin/scripts/probability_csmith_args.py $k $c`
...
function save_result {
    ...
    r="pk1(r3)&$m_tc&$cb1&$cb2&$cb3&$cb&$to1&$to2&$to3&$to&$wc \\\\"
    echo "$r" >> /project/alipour/rabin/main/result482.txt
    echo "$r"
}

cd /project/alipour/rabin/main/gcc482/
mkdir pk1; cd pk1
