#!/bin/bash
#SBATCH -J sb_csmith_r1
#SBATCH -o sb_csmith_r1.o%j
#SBATCH -t 06:00:01
...
function compile_tc {
    ...
    if [ $? -ne 0 ]; then
        timeout $tout2 /project/alipour/rabin/bin/gcc482-bin/bin/gcc -O$ol -I /project/alipour/rabin/bin/csmith230-bin/include/csmith-2.3.0 -w $tc -o $out 2> /dev/null
            if [ $? -ne 0 ]; then m_cb=$((m_cb+1)); echo > $out; echo $out >> CrashBug1.txt; return 1;  fi
    fi
...
function dump_output {
    for i in `ls $1-$2-$3.out`; do
        ...
        if [ $? -ne 0 ]; then
            ...
            if [ $? -ne 0 ]; then m_to=$((m_to+1)); echo > $i.txt; echo $i >> TimeOut1.txt; return 1; fi
        fi
    done
}
