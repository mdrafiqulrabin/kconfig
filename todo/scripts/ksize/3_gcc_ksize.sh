...
m_path_gcc="/project/alipour/rabin/kconfig/data/ksize/gcc_n"
m_version=""
m_run=""
m_tc=0
...
function result {
    ...
    res="$m_version & $m_run & $m_tc & $cb0 & $cb3 & $cb03 & $cb & $to0 & $to3 & $to03 & $to & $mc  \\\\"
    echo "$res" >> "$m_path_gcc/$m_version/result.txt"
    #echo "$res"
}

function init {
    m_path_input="$m_path_csmith/$m_run"
    m_path_output="$m_path_gcc/$m_version/$m_run"
